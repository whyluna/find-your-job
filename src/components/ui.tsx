...
export function TextInput({ className, ...props }: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      className={cn(
        "h-8 w-full rounded-lg border border-slate-200/90 bg-white px-2.5 text-[13px] text-slate-900 placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none focus:ring-[3px] focus:ring-indigo-500/10 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:focus:ring-indigo-400/10",
        className,
      )}
      {...props}
    />
...
export function Select({ className, children, ...props }: SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      className={cn(
        "h-8 w-full appearance-none rounded-lg border border-slate-200/90 bg-white px-2.5 text-[13px] text-slate-900 focus:border-indigo-400 focus:outline-none focus:ring-[3px] focus:ring-indigo-500/10 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100",
        className,
      )}
      {...props}
    >
...
export function Segmented<T extends string>({
  ...
  return (
    <div className="inline-flex items-center rounded-[9px] bg-slate-100 p-[2px] dark:bg-slate-800">
      {options.map((o) => (
        <button
          key={o.value}
          onClick={() => onChange(o.value)}
          className={cn(
            "flex h-7 items-center gap-1.5 rounded-[7px] px-3 text-[13px] font-medium transition-all",
            value === o.value
