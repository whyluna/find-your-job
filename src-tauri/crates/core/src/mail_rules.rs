...
const RULES: &[(&str, &[&str], &str)] = &[
    ("ASSESSMENT_INVITED", &["测评", "评价邀请", "职业性格", "ai 测评", "talent"], "测评/性格测试关键词"),
    ("WRITTEN_INVITED", &["笔试", "在线考试", "考试邀请", "编程题", "行测"], "笔试/在线考试关键词"),
    ("INTERVIEW_NOTE", &["面试邀请", "面试通知", "面试安排", "视频面试"], "面试通知关键词"),
    ("INTENT_LETTER", &["意向书", "录用意向"], "意向书关键词"),
    ("OFFER", &["offer", "录用通知", "聘用通知", "入职通知"], "offer 关键词"),
    ("RESUME_FAIL", &["简历未通过", "很遗憾", "未能通过筛选", "不匹配"], "拒绝/筛选未过关键词"),
...
fn extract_deadline_hint(text: &str) -> Option<String> {
    ...
    for i in 0..chars.len() {
        if chars[i] == '月' && i > 0 && i + 2 < chars.len() {
            let month: String = chars[..i].iter().rev().take_while(|c| c.is_ascii_digit()).collect::<Vec<_>>().into_iter().rev().collect();
            ...
            // 语境检查：截止词
            let has_deadline_ctx = ["截止", "之前", "前完成", "有效期"].iter().any(|k| text.contains(k));
            if has_deadline_ctx {
                let now = Utc::now();
                ...
                if (1..=12).contains(&m) && (1..=31).contains(&d) {
                    ...
                    if m < now.month() { year += 1; } // 已过月份视为明年
...
pub fn classify(mail: &MailInput, known_companies: &[(String, String, Vec<String>)]) -> Option<Suggestion> {
    ...
    for (name, website, aliases) in known_companies {
        ...
        if aliases.iter().any(|a| !a.is_empty() && (from_domain.contains(a.as_str()) || mail.subject.contains(a.as_str()))) {
            company_hint = Some(name.clone());
            reason_domain = true;
            break;
        }
    ...
    let et = event_type?;
    let deadline_hint = if et.0.ends_with("_INVITED") { extract_deadline_hint(&text) } else { None };
...
mod tests {
    ...
    fn companies() -> Vec<(String, String, Vec<String>)> {
        vec![
            ("腾讯".into(), "careers.qq.com".into(), vec!["tencent.com".into()]),
    ...
    fn assessment_invite_with_deadline() {
        let m = mail("【腾讯】职业测评邀请", "no-reply@tencent.com", "请于 9月10日 前完成在线测评，链接 3 天内有效。");
    ...
    #[test]
    fn interview_invite() {
        let m = mail("面试安排", "campus@nowcoder.com", "您已通过简历筛选，请参加视频面试");
        let s = classify(&m, &companies()).unwrap();
    ...
    fn rejection_requires_context() {
        ...
        let m2 = mail("应聘结果通知", "hr@example.com", "很遗憾，您的简历未能通过筛选");
